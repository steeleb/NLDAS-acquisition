...
### Set timeframe
###########################################################
out.ts = seq.POSIXt(as.POSIXct('2018-01-01 00:00:00',tz = 'GMT'),as.POSIXct('2019-12-31 23:00',tz='GMT'),by = 'hour')
...
for (i in 1:length(out.ts)) {
  ...
  URL <- paste('http://hydro1.sci.gsfc.nasa.gov/daac-bin/OTF/HTTP_services.cgi?',
               'FILENAME=%2Fdata%2Fs4pa%2FNLDAS%2FNLDAS_FORA0125_H.002%2F',yearOut,'%2F',doyOut,'%2FNLDAS_FORA0125_H.A',yearOut,monthOut,dayOut,'.',hourOut,'.002.grb&',
               'FORMAT=bmV0Q0RGLw&BBOX=',extent[2],'%2C',extent[1],'%2C',extent[4],'%2C',extent[3],'&',
               'LABEL=NLDAS_FORA0125_H.A',yearOut,monthOut,dayOut,'.',hourOut,'.002.2016116144611.pss.nc&',
               'SHORTNAME=NLDAS_FORA0125_H&SERVICE=SUBSET_GRIB&VERSION=1.02&DATASET_VERSION=002',sep='')
  ...
  lk <- URL
  # r <- GET(lk,
  #          authenticate("hilarydugan@gmail.com", "Earthdata2017"),
  #          path = "~/Desktop")
  ...
  curl::handle_setopt(
    handle = h,
    httpauth = 1,
    userpwd = "hilarydugan@gmail.com:Earthdata2017"
  )

  # resp <- curl::curl_fetch_memory(lk, handle = h)
  resp <- curl::curl_fetch_disk(lk, path = paste('~/Documents/MendotaRawData/',filename,'.nc',sep=''), handle = h)
